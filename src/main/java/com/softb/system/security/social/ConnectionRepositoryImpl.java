...
/**
 * Implementação para o Spring Social {@link ConnectionRepository}. Utiliza o
 * {@link UserSocialConnectionRepository} para recuperar as informações do
 * repositório local.
 * 
...
public class ConnectionRepositoryImpl implements ConnectionRepository {
	...
	public List<Connection<?>> findConnections(String providerId) {
		...
		List<UserSocialConnection> userSocialConnectionList = this.userSocialConnectionRepository.findByUserIdAndProviderId(userId,
				providerId);
		for (UserSocialConnection userSocialConnection : userSocialConnectionList) {
	...
	public MultiValueMap<String, Connection<?>> findConnectionsToUsers(MultiValueMap<String, String> providerUsers) {
		...
		for (Iterator<Entry<String, List<String>>> it = providerUsers.entrySet().iterator(); it.hasNext();) {
			...
			List<UserSocialConnection> userSocialConnections = this.userSocialConnectionRepository.findByProviderIdAndProviderUserIdIn(
	...
	public Connection<?> getConnection(ConnectionKey connectionKey) {
		UserSocialConnection userSocialConnection = this.userSocialConnectionRepository.findByUserIdAndProviderIdAndProviderUserId(userId,
	...
	public void addConnection(Connection<?> connection) {
		...
		if (socialAuthenticationService.getConnectionCardinality() == ConnectionCardinality.ONE_TO_ONE
			...
			List<UserSocialConnection> storedConnections = this.userSocialConnectionRepository.findByProviderIdAndProviderUserId(connection
					.getKey().getProviderId(), connection.getKey().getProviderUserId());
			if (storedConnections.size() > 0) {
				// not allow one providerId/providerUserId connect to multiple
				// userId
	...
	private Connection<?> buildConnection(UserSocialConnection userSocialConnection) {
		ConnectionData connectionData = new ConnectionData(userSocialConnection.getProviderId(), userSocialConnection.getProviderUserId(),
				userSocialConnection.getDisplayName(), userSocialConnection.getProfileUrl(), userSocialConnection.getImageUrl(),
